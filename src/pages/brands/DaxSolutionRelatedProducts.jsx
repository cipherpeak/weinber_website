...
function DaxSolutionRelatedProducts() {
    ...
    const fetchShowcaseProducts = async () => {
      try {
        ...
        if (data && data.length > 0) {
          const daxSolutionsProducts = data.filter((p) => p.brand === "Dax Solutions");
          const mapped = daxSolutionsProducts.slice(0, 4).map(p => ({
            id: p.id,
...
export default DaxSolutionRelatedProducts;
