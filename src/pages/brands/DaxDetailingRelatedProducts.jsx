...
function DaxDetailingRelatedProducts() {
    ...
    const fetchShowcaseProducts = async () => {
      try {
        ...
        if (data && data.length > 0) {
          const daxDetailingProducts = data.filter((p) => p.brand === "Dax Detailing");
          const mapped = daxDetailingProducts.slice(0, 4).map(p => ({
            id: p.id,
...
export default DaxDetailingRelatedProducts;
